...
class PsiLocatorTest : LightJavaCodeInsightFixtureTestCase() {
    fun testLocateReturnsMethodForLineInsideJavaMethod() = kotlinx.coroutines.runBlocking {
        val source = """
            class Sample {
                void ignored() {
                    int value = 1;
    ...
    fun testLocateSkipsLeadingWhitespaceOnLine() = kotlinx.coroutines.runBlocking {
        val source = """
            class Sample {
                int target(int input) {
                    return input * 2;
